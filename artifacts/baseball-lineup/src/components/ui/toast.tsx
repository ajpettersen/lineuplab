...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      // Bottom-anchored on every viewport. Originally mobile pinned
      // toasts to the top, which competed for tap space with the
      // team header. Bottom-positioned, the toast can't accidentally
      // intercept taps on the back button / kebab / nav, and slides
      // in from the bottom matching iOS-native expectations. Keep
      // safe-area padding so the chip clears the home indicator on
      // notched devices.
      "fixed bottom-0 right-0 z-[100] flex max-h-screen w-full flex-col p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:max-w-[420px]",
      className
    )}
    {...props}
  />
