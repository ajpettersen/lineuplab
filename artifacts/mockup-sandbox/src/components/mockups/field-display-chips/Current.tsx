    ...
    <div className="min-h-screen bg-[#0b1a35] flex items-center justify-center p-4 font-['Oswald']">
      <div className="w-full max-w-[420px] flex flex-col gap-2">
        ...
        >
          ...
          {/* Current chip style: yellow position badge ATTACHED to the LEFT of a navy name pill */}
          {PLAYERS.map((p) => (
            <div
              key={p.pos}
              className="absolute -translate-x-1/2 -translate-y-1/2 flex items-stretch shadow-md"
              style={{ left: `${p.x}%`, top: `${p.y}%` }}
            >
              <div className="bg-[#f5b800] text-[#0b1a35] text-[10px] font-bold px-1.5 flex items-center rounded-l-sm">
                {p.pos}
              </div>
              <div className="bg-[#0b1a35] text-white text-[11px] font-semibold px-2 py-1 rounded-r-sm whitespace-nowrap">
                {p.name}
              </div>
            </div>
          ))}
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-2">
              {BENCH.map((n) => (
                <span
                  key={n}
                  className="text-[11px] text-white border border-white/30 rounded-sm px-2 py-0.5"
                >
                  {n}
                </span>
              ))}
