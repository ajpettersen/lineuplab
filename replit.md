...
- **Position Locks**: Pinning players to specific positions/innings, with validation and generator integration. The "Add Position Lock" dialog uses a **multi-inning checkbox grid** with quick "All / None" toggles, so a coach can lock a player at one inning, several innings, or every inning of the game in a single action. Server `POST /api/games/:id/locks` accepts `inning` as `null` (all innings), a single `number` (legacy), or a `number[]` (multi-pick) — all three insert one row per inning under the same conflict-checked transaction.
- **AI Assistant**: Natural language querying, lineup regeneration, and player removal, with conflict resolution against existing locks. Three intents: `answer` (Q&A), `regenerate` (pin players to slots → re-run generator), and `remove` (pull players out of the lineup entirely — for injuries / early departures, e.g. "Henry got hurt, take him out"). The `remove` intent strips every entry for the named player(s) from the displayed lineup (preview > edited > saved), also clears those players from AI pin memory so a subsequent regenerate doesn't resurrect them, and surfaces a "still has position locks" hint if any of them have explicit locks. Per-game **AI memory** persists assistant-issued pins across calls (table `ai_pinned_assignments`, unique on `gameId+playerId+inning` and on `gameId+inning+position` where position!='Bench'). Each AI regenerate merges new pins on top of memory pins (new wins on conflict by player+inning OR inning+position) and writes the union back. Locks always win over both. UI shows a "Remembering N pins" indicator with a "Reset" button (`DELETE /api/games/:id/ai-pins`). `GET /api/games/:id/ai-pins` returns `{pins, count}`.
...
- **Innings by Position Tally**: Live updates on player position counts per game. Includes an "Out" column showing innings the player has no entry for (e.g. they showed up late, left early, or a photo import didn't pick them up), so each row's total equals the game length. Each row also has a **trash icon** at the end to remove that player from the entire displayed lineup in one click (for injuries / mid-game departures). For better discoverability, the lineup grid header **also** surfaces a destructive "Remove [first name]" button whenever a player chip is selected (tap-to-select). Both surfaces call the same `handleRemovePlayerFromLineup` handler, which operates on whichever lineup is on screen — preview / edited / saved — and seeds an editable copy when needed; field cells the player held become empty slots the coach can refill or save as-is. Both surfaces are hidden in print.
...
- **Multi-Coach Team Sharing**: A head coach can mint single-use invite links from Settings → Coaches and share them via any channel. Whoever signs in and accepts gets full **co-owner** access to the team's roster, schedule, lineups, AI memory, and constraints. Coaches can belong to multiple teams (e.g. their own team plus one they assist with) and switch between them via a header dropdown that only renders when there are 2+ teams. The dropdown clears the React Query cache on switch so all data refetches under the new scope. Architecture: existing tables stayed user-keyed; a new `req.ownerUserId` (set by middleware `resolveTeamContext` after `requireAuth`) is the data scope, while `req.userId` remains the actual signed-in person. Three new tables — `team_invites` (token, expiresAt, revokedAt, acceptedAt), `team_memberships` (unique per owner+member), `user_active_team` (per-user pointer). Invite acceptance is race-safe: the guarded `UPDATE … WHERE acceptedAt IS NULL` runs **first** in the transaction so a losing concurrent request performs zero writes before bailing with "Invite already used". The /join/:token page lives outside `ProtectedApp` — signed-out visitors stash the token in `sessionStorage` and `PendingInviteRedirect` (in App.tsx) bounces them back after Clerk sign-in. Coach removal handles the "I removed myself" case by clearing the user's `user_active_team` pointer; the `resolveTeamContext` middleware then falls back to their own team on the next request.
- **Effective Game Status**: The DB column `games.status` is one of `"upcoming" | "completed" | "cancelled"` and stays "upcoming" until the coach explicitly marks complete. The UI derives an **effective** status (`"upcoming" | "past" | "completed" | "cancelled"`) client-side via `src/lib/game-status.ts` (`effectiveStatus`, `isTrulyUpcoming`) by comparing `gameDate` to `Date.now()` so a stored-but-past game shows as an amber "Past" badge instead of pretending to still be on the schedule. Used by Games page bucketing + badge, Dashboard "Upcoming Games" sidebar, game-detail header badge, and the "Copy from previous game" picker. The Mark Complete button intentionally still keys off raw `status === "upcoming"` so it appears for both future and past-unrecorded games. Status is never written as `"past"`; the form only offers the three storable values.
...
- `team_invites`: Single-use invite links for assistant coaches (token, label, expiresAt, revokedAt, acceptedAt, acceptedByUserId). Owner-scoped.
- `team_memberships`: Records which assistant coaches belong to which head coach's team. Unique on (ownerUserId, memberUserId). Head coach is implicit and not stored here.
- `user_active_team`: Per-user pointer to the currently-active team's owner. Lets a coach who belongs to 2+ teams pick which one they're working on.
