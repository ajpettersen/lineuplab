...
- **Permission Tiers**: `team_memberships.permission` is `full | partial | upload | view` (linear rank: view < upload < partial < full). Server enforces with `assertPermission(level)` middleware (see `lib/permissions.ts`); web hides write controls via `usePermission()`. Owner row is locked to `full`. The `upload` tier is for a stat-keeper / "GameChanger" parent — read-only everywhere EXCEPT `/games/:id/box-score*` which gates at `upload` so partial+full pass too. UI labels in `coaches-card.tsx` map tiers to suggested role titles (Head Coach / Assistant Coach / GameChanger / Read-only).
