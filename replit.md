...
- **Team-Name Onboarding Modal**: `<TeamNamePrompt>` (mounted in `Layout` next to `CoachProfilePrompt`) auto-opens on sign-in when `teamName === "My Team"` or `teamShortName === "Team"` (the seeded defaults). Forces a real name before the head coach can use the app; suppressed for non-`full` permission coaches (they wait for the head coach).
...
- **Onboarding Wizard**: Head coaches whose `team_settings.onboardingCompletedAt` is null get redirected from any route to `/welcome` (a focused 7-step wizard rendered outside the app `Layout`): welcome → coach profile → team identity → colors → roster → invite coaches (by email) → tour. Final step calls `POST /api/team-settings/complete-onboarding` and bounces home. The `OnboardingGate` in `App.tsx` skips the wizard for assistant coaches (`!ctx.isOwner`), master admins (`isMasterAdmin`), `/join/:token`, and the field display URL. Existing users automatically see it on next sign-in (the column is null for legacy rows). Signed-out `/` renders `pages/landing.tsx` (free-marketing landing) instead of bouncing to `/sign-in`. Invite emails are stored on `team_invites.invitedEmail` with a `sentEmailAt` timestamp reserved for a future email-delivery worker — today the coach still copies the join link.
- **Offline / PWA (Phase 1)**: `vite-plugin-pwa` registers a service worker (`generateSW`/Workbox, `registerType: "autoUpdate"`) that precaches the app shell + SPA-falls-back navigation requests to `index.html`. Runtime caching is intentionally limited to Google Fonts — `/api`, `/sign-in`, `/sign-up` are denylisted from the navigation fallback and never SW-cached, because data caching is owned by React Query's `PersistQueryClientProvider` (see `src/lib/query-persister.ts` — IndexedDB via `idb-keyval`, 14-day `maxAge`, `gcTime` matched, `buster: "v1.0.0"`). Per-user safety leans on the existing `ClerkQueryClientCacheInvalidator` which calls `qc.clear()` on user change (the persister mirrors that to IndexedDB on the next throttled write). `<SyncStatusChip>` (header right cluster) shows "Loading" while rehydrating and "Offline" when `navigator.onLine` is false; otherwise hidden. `<InstallPwaPrompt>` (mounted in `Layout`) shows a one-time iOS-only "Add to Home Screen" banner (re-shows after 60d if dismissed). `index.html` carries the `apple-mobile-web-app-*` + `theme-color` meta tags. Phase 2 (offline write queue + true server-reachability heartbeat) is not yet implemented — the chip currently reflects browser network status only, and writes fail loudly if attempted offline. See `vite.config.ts` `VitePWA(...)`.
