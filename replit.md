...
- **Per-game Competitiveness** (`games.competitiveness`, nullable int 0-100): per-game dial for the BATTING ORDER ONLY (defense untouched). `null` = legacy behavior (gameType branch + global fairness dial). When set, blends `w=competitiveness/100` between an equitable order (ascending PA) and a competitive order (OPS arrangement). **UI is consolidated into the Generate Lineup dialog** (`game-detail.tsx`) — it sits directly under the global Fairness Dial as an optional checkbox-gated `Slider` ("Use a different balance for just this game"), so the per-game override and the global default live in ONE place. `handleGenerate` seeds the toggle from `game.competitiveness` on open and PATCHes `games.competitiveness` (number or `null`) right before generating — mirroring how the fairness dial persists `global_equity_weight` — then invalidates the game query. The standalone competitiveness slider was REMOVED from new-game/edit-game (it was a redundant second knob pointing the same fair→competitive direction as the Fairness Dial). Off → sends `null`.
- **First-inning start balancing** (`lib/lineup-generator.ts` `playerSeasonStarts` + `startBalanceBonus`): inning 1 carries no in-game bench history, so its fairness term is 0 for everyone and the opening lineup otherwise falls to crowded preferred-position contests + input order — the SAME kids keep starting on the bench game after game (even when their season-long bench rate is fair). The route (`lineups.ts`) mines a background per-player `{starts, gamesPlayed}` stat from saved lineups (`starts` = past games on the field — not benched — in inning 1; excludes the game being generated), and the generator converts it to a start-RATE and nudges below-average starters onto the field in inning 1 ONLY. **Gated on `equity > 0.5`** (the balance dial leaning fair) — so it's ON for untyped/league games (global equity ~80, league forces 70) and OFF for tournament/competitive games (gameType forces equity 10) where the best players keep starting. Nudge strength ramps 0→~50 across equity 0.5→1.0 (≈+30 spread at typical 0.8 — on par with the must-play boost, so it flips the opening contest without overriding pins/bench/must-play). Brand-new players (0 gamesPlayed) stay neutral. NOT wired into the AI assistant regen route. No new schema, no UI (intentionally a background stat).
...
- **Stakes Ladder** (Field Display intensity): ONE dugout screen with five escalating game-context rungs derived in `field-display.tsx` into `fieldMode` and exposed as `data-fd-mode` on the root: `league < pool < bracket < champ < champ-elevated`. Priority: `champ-elevated` (championship game AND our team currently leads — the momentum peak, reusing the take-the-lead beat) > `champ` (championship) > `bracket` (tournament `bracketStage==="bracket"`) > `pool` (any other tournament game) > `league`. A `.fd-stakes-banner` ribbon (aria-hidden) sits above the header for non-league rungs; `.fd-champ-flecks` falling gold particles render only at the peak.
- **Team-colored chrome, gold reserved for trophies**: the Field Display is themed in two layers. (1) Dark chrome SURFACES (header bar, panels, position chips, borders, field surround) derive from the TEAM PRIMARY via `--fd-*` tokens in `index.css` (`--fd-bg/--fd-panel/--fd-panel-deep/--fd-chip/--fd-chip-soft/--fd-border`), each `color-mix(in srgb, hsl(var(--primary)) N%, <near-black>)` with a plain-hex fallback line first for iPad Safari <16.2 — so a royal-blue/maroon/forest team's whole screen reads in their color instead of generic slate. Use `bg-[var(--fd-panel)]` etc., never re-add hardcoded `#0f172a`/`#0b1a35`. (2) HIGHLIGHTS are painted in the TEAM `accent` token — NOT hardcoded gold (`broadcast-gold` was migrated to `accent` in `field-display.tsx`/`score-stepper.tsx`). Default teams (whose accent IS gold) look unchanged; custom teams now theme. Gold returns ONLY at the championship rungs via a single CSS lever in `index.css`: `[data-fd-mode="champ"],[data-fd-mode="champ-elevated"]{ --accent:42 95% 55%; --accent-foreground:220 85% 18%; }` re-points the accent token for the whole subtree so every themed `accent` utility (header border, score numerals, active batter, end-game buttons, tourney chyron) becomes trophy metal with no per-element overrides. Nearest-scope var wins, so this does not conflict with `TeamThemeApplier` setting `--accent` on `<html>`. Per-position chip palette (`POSITION_ACCENT`) is left as functional color-coding (battery/infield/outfield), not brand gold. **`<TeamThemeApplier>` is mounted ONCE at the signed-in app root (`App.tsx` `ProtectedApp`, inside `<Show when="signed-in">`, above the route `<Switch>`) — NOT inside `<Layout>` — so full-screen surfaces that render outside the app shell (Field Display, `/welcome` onboarding) still inherit team colors. Any new theming provider must mount at this root, not in Layout, or those surfaces silently fall back to default navy/gold.**
- **Lineup panel POS labels**: each batting row in the Field Display lineup panel shows the player's current-inning fielding position (themed `accent`), derived from a `posByPlayer` memo (inverse of `fieldByPos` for `currentInning`). Bench/unplaced players show no label (real lineup data only — intentionally NOT a fake at-bat indicator).
- **Championship flag & confetti**: driven per game by `games.isChampionship` (notNull, default false), set via "championship game" checkbox in Edit Game (tournament games only; PATCH rejects it on non-tournament games). The kebab toggle is a SESSION-scoped override (`champOverride`, reset when `id` changes — a global toggle used to leak gold onto later games). `champ`/`champ-elevated` keep the `.fd-championship` "party, not pressure" frame glow. One-shot welcome confetti fires once per game when championship mode turns on (latch keyed on route `id`, gated on `game?.id === id`). All animations in `src/index.css` honor `prefers-reduced-motion`.
