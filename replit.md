...
- **Tie-game handling**: A completed game whose `ourScore === opponentScore` is classified as a tie (T), not a loss. The dashboard "Record" card renders `W-L` while no ties exist and switches to `W-L-T` as soon as one does. Tie badges (amber) render in `dashboard.tsx` `ScoreBadge`, the games-list row badge in `games.tsx`, and the game-detail header badge in `game-detail.tsx` — keep these three call sites in sync if the badge format changes.
- **Top-bar nav density**: The desktop header nav (`components/layout.tsx`) uses tight spacing — `gap-2` on the right cluster, `gap-0.5` between nav items, `px-2.5 py-2` link padding, `text-[13px]` with `tracking-[0.1em]`, and the active-route gold underline insets matched at `left-2.5 right-2.5` — so 6+ tabs (Dashboard, Roster, Games, Practices, Stats, History, Settings, Admin…) fit on a 13" laptop without triggering the horizontal auto-scroller.
- **Field-display short team names**: `pages/field-display.tsx` defines a `shortenTeamName()` helper that strips trailing youth-baseball "modifier" tokens (color words from a fixed set, plus level codes matching `/^(?:\d{1,2}[A-Z]{1,3}|U\d{1,2}|A{1,3}|B|C)$/`, case-insensitive) from the END of a name, stopping at the first non-modifier token. So "Edina Green 10AA" → "Edina", "Minnetonka 10AA Blue" → "Minnetonka", but "Lakeville South 12U" → "Lakeville South" (compound city survives) and "Wilsonville Blazers" stays intact. Always returns at least the first token. Our team falls back through `teamShortName → shortenTeamName(teamName) → teamName`; opponent uses `shortenTeamName(opponent)` directly. The full official name is preserved in each `<span title>` tooltip.
