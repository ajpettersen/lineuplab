...
- **Season Batting Import** (`POST /api/batting/extract` → review → `POST /api/batting/import-season`): the Season Stats "Import Stats" button accepts a **GameChanger CSV export** OR an image/PDF. The route branches on file type (`.csv` ext or `csv` mimetype): CSV is parsed DETERMINISTICALLY (no AI charge) by `extractGameChangerBatting` in `routes/batting.ts`; images/PDF go through the gpt-5.2 vision path (`chargeAiCall` only fires there). Both return `{ extracted, unmatched? }` into the same review-then-save preview. `import-season` stores FULL season totals on the manual `batting_stats` row + stamps `seasonImportedAt` cutoff so it overrides prior box-score rollups while future games accumulate on top. **GameChanger CSV gotcha**: 2-row header — row 1 = section markers (Batting/Pitching/Fielding), row 2 = column names; H/R/BB/SO/SB/HBP appear in BOTH batting & pitching sections, so batting reads are scoped to columns LEFT of the "Pitching" marker. Players matched by unique jersey number then normalized name; parsing stops at the Totals/Glossary rows; leading UTF-8 BOM stripped.
