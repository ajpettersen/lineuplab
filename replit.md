...
- **Web Push — Box-Score Reminders**: When `usesGameChanger=true` AND a coach opts in via Settings → Defaults → "Box-score reminder push", the API fires a single Web Push notification per game ~2h after `gameDate` reminding them to upload the box score. Backend: `web-push` lib signed with `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY`/`VAPID_SUBJECT` env vars (singleton in `api-server/src/lib/push.ts`); subscriptions live in `push_subscriptions` (PK on `endpoint`, FK to coach `userId`); `box-score-reminder-scheduler.ts` polls every 5 min with a single-flight guard, uses `UPDATE games SET boxScoreReminderSentAt=now() WHERE … AND boxScoreReminderSentAt IS NULL RETURNING *` as an atomic claim, fans out to every subscription belonging to coaches on the game's team, and prunes endpoints that return 404/410. `boxScoreReminderSentAt` is reset to `null` whenever a box score is (re-)imported so a delete + re-prompt cycle re-arms cleanly. Frontend: custom `push-sw.js` is `importScripts`-merged into the Workbox SW (vite-plugin-pwa) and handles `push` + `notificationclick` (postMessage navigate to existing tab, fallback `openWindow`); `usePushNotifications()` hook in `src/lib/push-notifications.ts` owns the capability/permission/subscribe state machine and detects iOS-not-installed-PWA up front. Routes: `GET /api/push/vapid-public-key`, `POST /api/push/subscribe`, `POST /api/push/unsubscribe` (hand-rolled, no OpenAPI). If VAPID env vars are missing the scheduler + routes degrade gracefully to no-op.
