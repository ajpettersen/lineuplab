...
- **Field Display Offline Support**: The dugout-fence iPad keeps working when park WiFi drops. Three layers: (1) **Reads survive offline** — every successful game/lineup query is mirrored to `localStorage` (keys `fd-game-cache-v1:<id>` / `fd-lineup-cache-v1:<id>`); on mount the React Query cache is hydrated from localStorage via `initialData` so the page renders the last-known lineup instantly, even with no network and even after a Safari refresh. (2) **Writes survive offline** — when the coach drag-drops with no connectivity, the optimistic UI update fires identically, but instead of POSTing we persist the desired final lineup snapshot to `localStorage` (key `fd-pending-save-v1:<id>`) and leave it there. We persist a SNAPSHOT not a queue: because the server uses REPLACE semantics on the entire lineup, the latest snapshot is always sufficient, so multiple offline drags coalesce into a single POST on reconnect. (3) **Auto-sync on reconnect** — a `useOnlineStatus()` hook listens to `navigator` `online`/`offline` events; when online flips true we re-call `flushSave()` which drains the persisted snapshot through the same single-flight save chain. The chain coalesces with newer drags safely. Conflict policy is **last-writer-wins with the iPad as the live source**: any phone edits made during the iPad's offline window are overwritten on reconnect (the dugout coach is the live source of truth during a game). Header status badge swaps from "Live" / "Just updated" to amber **"Offline"** or **"Offline · will sync"** when there are queued edits, with `data-online`/`data-pending` attributes for e2e assertions. `flushSave` distinguishes a clean offline-drop (silent retry) from a real server error (drop pending + toast + refetch); on a network drop mid-POST it restores `pendingLineupRef` only if no NEWER drag has set a fresher one (so we never overwrite a coach's most recent state with an older in-flight save). All localStorage access is wrapped in try/catch so private mode / quota errors silently degrade — the in-memory cache still works for the session, just not across refresh.
...
- **Field Display Score Stepper (manual scoreboard input)**: Until GameChanger integration lands, the dugout coach enters runs directly on the iPad via the header score area. Each side (ours/opponent) is a `<ScoreStepper>` with three input methods that all map to the same ±1 action — coach uses whichever fits muscle memory: (1) **tap chevron-up / chevron-down buttons** above and below the number — explicit, accessible, also covers desktop mouse + keyboard users; (2) **tap the number itself** = +1 (the overwhelmingly common case when our team scores — one-finger one-second action); (3) **swipe vertically on the number** — drag up = +1, drag down = -1 (the "push up or down on runs" gesture). `SWIPE_THRESHOLD = 24px` is the boundary between tap-as-tap and tap-as-swipe. `touch-action: none` on the number area prevents iOS Safari from stealing the vertical swipe as a page scroll, and `setPointerCapture` on pointerdown keeps the gesture coherent if the finger drifts off the number's bounding box. Score is floored at 0 — youth baseball doesn't have negative scores. For accessibility: `role="spinbutton"` + `aria-valuenow/min` so screen readers announce the score, plus `ArrowUp` / `ArrowDown` / `+`/`-` key bindings. Each tap fires `saveGamePatchOptimistically({ ourScore: next })` (or `opponentScore`) which: (a) updates React Query's game cache instantly via `setQueryData` (no flicker), (b) merges the field into a coalesced `pendingGamePatchRef` (so two rapid +1 taps on ourScore collapse to one PATCH, but ourScore + opponentScore taps both ride along in a single PATCH), (c) persists the merged patch to `localStorage` (key `fd-pending-game-patch-v1:<id>`), (d) flips `hasUnsyncedScore`, (e) kicks `flushGameSave()` — a parallel single-flight chain (separate `saveGameQueueRef`) that mirrors the lineup save chain's offline-aware logic exactly: skip POST when `navigator.onLine` is false, distinguish offline-drop (silent retry on reconnect) from real server error (toast + invalidate to roll back to server truth), and only clear the localStorage entry when no NEWER tap has replaced the in-flight value. The `useUpdateGame` hook is created with `networkMode: "always"` so React Query's built-in offline pause doesn't hang `mutateAsync` — our explicit `navigator.onLine` gate is the single source of truth. The unsynced badge in the header (`Offline · will sync`) is `hasUnsyncedLineup || hasUnsyncedScore`, so a coach with both pending drags AND pending score changes sees one combined indicator. On reconnect the online-flip effect drains both chains. `initialGame` hydrates from `localStorage` with the pending score patch already applied, so a refresh while offline shows the coach's pending score on first paint (no flicker to the stale server value). Conflict policy matches the lineup: last-writer-wins with the iPad as the live source.
